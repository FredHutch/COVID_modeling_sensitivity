...
while( $line=<INP>) {
    ...
    my $rds_file="../sens_data/$dist\_vei_$vac_eff_i\_ves_$vac_eff_s\_vep_$vac_eff_p\_sdmin_$sd_lower\_sdmax_$sd_upper\_rate_$vac_rate\_mut_$mut_inf\_trigmin_$sd_loosen\_trigmax_$sd_tighten\_trigperc_0_cover_$coverage\_import_$imports\.rds";
    print("Filename: $rds_file\n");

    #if (! -e $rds_file) {
	print("Running: Rscript one_run_from_Oct.R $dist $age_code $vac_eff_i $vac_eff_s $vac_eff_p $vac_rate $mut_inf $sd_lower $sd_upper $sd_loosen $sd_tighten 0 $coverage $imports\n");
	system("Rscript one_run_from_Oct.R $dist $age_code $vac_eff_i $vac_eff_s $vac_eff_p $vac_rate $mut_inf $sd_lower $sd_upper $sd_loosen $sd_tighten 0 $coverage $imports");
    #} else {
#	print "Skipped creation of $rds_file\n";
#    }
    system("Rscript one_run_table.R $dist $vac_eff_i $vac_eff_s $vac_eff_p $vac_rate $mut_inf $sd_lower $sd_upper $sd_loosen $sd_tighten 0 $coverage $imports");
    system("tail -n 1 sens_out/$dist\_vei_$vac_eff_i\_ves_$vac_eff_s\_vep_$vac_eff_p\_sdmin_$sd_lower\_sdmax_$sd_upper\_rate_$vac_rate\_mut_$mut_inf\_trigmin_$sd_loosen\_trigmax_$sd_tighten\_trigperc_0_cover_$coverage\_import_$imports\.csv >> sens_$inpfile");
}
close(INP);
