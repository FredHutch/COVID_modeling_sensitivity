...
start_date_opt<-as.numeric(args[2])
vei<-args[3] # example: 0, 0.25, etc.
rate<-args[4] # example: 5000
vac_fract<-args[5] # example: 0.3 (30%), 0.5(50%), etc.
...
vac_exp_rate=as.numeric(args[9])

source("covid-model.R")
source("kc_read-data.R")

tot_vac= as.integer((as.numeric(vac_fract) * the_pop + 5000)/10000) * 10000
...
suffix=paste0(dist,"_start_",start_date_opt,"_vei_",vei,"_sdmin_",min_sd,"_rate_",rate,"_tot_",tot_vac,"_trigs_",trig_min,"_",trig_max,"_",args[9])
infile=paste0("../data/",suffix,".rds")

suffix=paste0(dist,"_start_",start_date_opt,"_vei_",vei,"_sdmin_",min_sd,"_rate_",rate,"_fract_",vac_fract,"_trigs_",trig_min,"_",trig_max,"_",args[9])
...
# vaccination date
if (start_date_opt==1) {
    start_date = 366 + yday("2021-1-01")
}
if (start_date_opt==2) {
    start_date = 366 + yday(ymd("2021-3-01"))     # Start of vaccination protocol
}
...
write("scenario,start,min_sd,ve_s,ve_p,ve_i,vac_rate,vac_total,cases since vax,%case reduct,max daily cases,hosp since vax,%hosp reduct,max current hosp,deaths since vax,%death reduct,max daily deaths,avg SD,avg child SD,days at max SD,%reduct days at max,max Reff,min Reff, avg Reff,tot cases,tot hosp,tot deaths,loosen,tighten,cases since vax yr2,%case reduct yr2,max daily cases yr2,hosp since vax yr2,%hosp reduct yr2,max current hosp yr2,deaths since vax yr2,%death reduct yr2,max daily deaths yr2,avg SD yr2,avg child SD yr2,days at max SD yr2,%reduct days at max yr2,max Reff yr2,min Reff yr2, avg Reff yr2,tot cases yr2,tot hosp yr2,tot deaths yr2,vac_exp_rate", file = outfile, append=FALSE)
for (i in 1:nrow(base_results))
{
    ...
    write(paste(scen_name,start_date_opt,min_sd,ve_s,ve_p,ve_i,vrate,vtot,
