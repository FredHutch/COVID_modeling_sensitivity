...
new_strain_fact<-as.numeric(args[4])
min_sd<-args[5] # how far to relax SD
max_sd<-args[6] # how far to relax SD
...
trig_perc<-args[9] # bi-weekly case & hosp rate percent rise/drop to tighten/loosen SD (after "new_check_date")
cover<- args[10] # age-group vax coverage (fraction)
imports<- args[11] # daily new mutation imports
vac_exp_rate=0
...
print("total infs at year 1...")
print(paste("No vax: ",scenarios_base$inf[end_idx,1],"(",100*scenarios_base$inf[end_idx,1]/the_pop,"%)"))
for (i in 2:ncol(scenarios_base$inf))
{
    print(paste("VAX",i-1,":",scenarios_base$inf[end_idx,i],"(",100*scenarios_base$inf[end_idx,i]/the_pop,"%)"))
}
print("total cases at year 1...")
print(paste("No vax: ",scenarios_base$cases[end_idx,1],"(",100*scenarios_base$cases[end_idx,1]/the_pop,"%)"))
for (i in 2:ncol(scenarios_base$cases))
{
    print(paste("VAX",i-1,":",scenarios_base$cases[end_idx,i],"(",100*scenarios_base$cases[end_idx,i]/the_pop,"%)"))
}
print("total deaths at year 1...")
print(paste("No vax: ",scenarios_base$deaths[end_idx,1],"(",100*scenarios_base$deaths[end_idx,1]/the_pop,"%)"))
for (i in 2:ncol(scenarios_base$deaths))
{
    print(paste("VAX",i-1,":",scenarios_base$deaths[end_idx,i],"(",100*scenarios_base$deaths[end_idx,i]/the_pop,"%)"))
}
print("days at SD max during year 1...")
print(paste("No vax: ",calc_days_at_max(scenarios_base$sd_2, start_idx, end_idx)[1]))
for (i in 2:ncol(scenarios_base$inf))
{
    print(paste("VAX",i-1,":",calc_days_at_max(scenarios_base$sd_2, start_idx, end_idx)[i]))
}
...
write("scenario,min_sd,max_sd,ve_s,ve_p,ve_i,vac_rate,new_strain_fact,loosen,tighten,trig percent,coverage,import rate,cases since vax,%case reduct,max daily cases,hosp since vax,%hosp reduct,max current hosp,deaths since vax,%death reduct,max daily deaths,avg SD,avg child SD,days at max SD,%reduct days at max,max Reff,min Reff, avg Reff,tot cases,tot hosp,tot deaths,tot_infs", file = outfile, append=FALSE)
for (i in 1:nrow(base_results))
{
	...
	ve_s=interventions[i-1,1]
	ve_p=interventions[i-1,2]
	ve_i=vei
	vrate=vac_rate
	scen_name = dist
