#suffix="nhd_0.2_to_0.5_eff_0.7_5k" (now passed on Rscript command line)
...
vac_eff_p = as.numeric(p_eff)

suffix=paste0(scen,"_vep_",vac_eff_p,"_SD_",min_sd,"_to_",max_sd,"_rate_",rate,"_tot_",tot_vac,"_trigs_",trig_min,"_",trig_max)

infile=paste0("../data/scenarios_vacc_",suffix,".rds")
outfile=paste0("out/results_table_",suffix,".csv")
...
# one year later
end_date = 366 + yday("2021-12-31") -1
...
print(end_idx)
print(scenarios_base$deaths[end_idx,])
...
write("scenario,min_sd,max_sd,ve_s,ve_p,ve_i,vac_rate,vac_total,cases since vax,%case reduct,max daily cases,hosp since vax,%hosp reduct,max current hosp,deaths since vax,%death reduct,max daily deaths,avg SD,avg child SD,days at max SD,%reduct days at max,max Reff,min Reff, avg Reff,tot cases,tot hosp,tot deaths,loosen,tighten", file = outfile, append=FALSE)
for (i in 1:nrow(base_results))
{
    ...
    write(paste(scen_name,min_sd,max_sd,ve_s,ve_p,ve_i,vrate,vtot,
