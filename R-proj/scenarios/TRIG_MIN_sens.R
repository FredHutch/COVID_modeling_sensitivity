...
interventions = matrix(c(
		((trig_max + 20)/2) * the_pop / 100000,((trig_max - 20)/2) * the_pop / 100000,1,
		((trig_max + 60)/2) * the_pop / 100000,((trig_max - 60)/2) * the_pop / 100000,1,
		((trig_max + 100)/2) * the_pop / 100000,((trig_max - 100)/2) * the_pop / 100000,1),
                       byrow = TRUE, nrow = 3)
...
suffix=paste0(scen,"_sdmin_",min_sd,"_sdmax_",max_sd,"_rate_",rate,"_trig_max_",trig_max)
print(suffix)
...
plot_scenarios(doy, cases, yday(the_data$date), the_data$cases, 
                ...
                y_lab = "Daily Diagnosed Cases", x_lim = NULL, col_pal = cols, col_idx = 1:3,y_lim = c(0,1500),#lwd=lwds,
...
cases = apply(cases,2, diff)
plot_scenarios(doy, cases, yday(the_data$date), the_data$cases, 
                y_lab = "Daily Diagnosed Cases", x_lim = NULL, col_pal = cols, col_idx = 1:3,y_lim = c(0,1500),
                delta = NULL, vaccination_date = vac_init_doy, calib_date = calib_doy, scenarios_out$sd_2[,1], NULL)
dev.off()
