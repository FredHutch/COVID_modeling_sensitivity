...
#Roll out = 2000, 5000, 10000: lockdown threshold=500, VEs = 90, VEi=0, VEsymp=0 -> y-axis = daily cases
#Roll out = 2000, 5000, 10000: lockdown threshold=500, VEs = 90, VEi=0, VEsymp=0 -> y-axis = daily hospitalizations
#Roll out = 2000, 5000, 10000: lockdown threshold=500, VEs = 90, VEi=0, VEsymp=0 -> y-axis = daily deaths
...
suffix=paste0(scen,"_vep_",vac_eff_pi,"_SD_",min_sd,"_to_",max_sd,"_trig_",trig_max,"_tot_",tot_vac)
print(suffix)
...
plot_scenarios(doy, cases, yday(the_data$date), the_data$cases, 
                ...
                y_lab = "Daily Diagnosed Cases", x_lim = NULL, col_pal = cols, col_idx = 1:4,y_lim = c(0,800),#lwd=lwds,
...
plot_scenarios(doy, deaths, yday(the_data$date), the_data$deaths, 
                #y_lab = "Daily Deaths", x_lim = c(startx,endx), col_pal = cols, col_idx = 1:4,y_lim = c(0,12),#lwd=lwds,
                y_lab = "Daily Deaths", x_lim = NULL, col_pal = cols, col_idx = 1:4,y_lim = c(0,12),#lwd=lwds,
               delta = NULL, vaccination_date = vac_init_doy, calib_date = calib_doy, scenarios_out$sd_2[,1], NULL)
#legend("topleft",
...
plot_scenarios(doy, inf, yday(the_data$date), NA, 
               y_lab = "Daily Infections", x_lim = x_lim, col_pal = cols, col_idx = 1:4,y_lim = c(0,8000),#lwd=lwds,
               delta = NULL, vaccination_date = vac_init_doy, calib_date = calib_doy, scenarios_out$sd_2[,1], NULL)
...
plot_scenarios(doy, cum_hosp, yday(the_data$date), NA, 
                y_lab = "Daily Hospitalizations", x_lim = x_lim, col_pal = cols, col_idx = 1:4,y_lim = c(0,100),#lwd=lwds,
               delta = NULL, vaccination_date = vac_init_doy, calib_date = calib_doy, scenarios_out$sd_2[,1], NULL)
dev.off()
